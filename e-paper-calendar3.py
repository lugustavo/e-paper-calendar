...
#picdir = "C:\Work\python\calendar\e-Paper\RaspberryPi_JetsonNano\python\pic"
libdir = "C:\\Work\\python\\calendar\\e-Paper\\RaspberryPi_JetsonNano\\python\\lib"
...
# Caminhos fixos
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
CREDENTIALS_FILE = os.path.join(BASE_DIR, "credentials_work-laptop.json")
...
def get_placeholder_image(prompt: str, size: str = "1024x1024") -> Image.Image:
    ...
    if os.path.exists('gpt-image.png'):
        return Image.open('gpt-image.png').convert("RGB")
    ...
    try:
        ...
        raise ValueError("Resposta da API não contém imagem válida.")

    except Exception as e:
        print(f"Erro ao gerar imagem com ChatGPT: {e}")
        return None
...
#
    #    title = "● Google Login ●"
    #    w, h = text_size(draw, title, font=font_title)
...
def draw_month_calendar(draw, ox, oy, w, h, now):
    ...
    for row in month_grid:
        for col, day in enumerate(row):
            ...
            if day:
                txt = str(day)
                txt_w, txt_h = text_size(draw, txt, day_font)
...
def draw_events(draw, ox, oy, w, h, items, page_index=0, total_pages=1):
    ...
    if not items:
        ...
        if placeholder:
            ...
            msg = "Dia livre"
            mw, mh = text_size(draw, msg, item_font)
...
def render_dynamic(base_img, page_index=0, page_size=3):
    ...
    #print(right_x, right_y, cal_h, right_w)
    ...
    #print (left_x+2, left_y+2, left_w-4, left_h-6)
    ...
    if args.dry_run:
        ...
        while True:
            try:
                now = datetime.now(tz)
                if now.date() != today:
                    # Novo dia → regenerar base estática
